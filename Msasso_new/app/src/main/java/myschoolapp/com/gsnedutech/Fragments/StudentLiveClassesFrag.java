...
public class StudentLiveClassesFrag extends Fragment {

    private static final String TAG = "SriRam -" + StudentLiveClassesFrag.class.getName();
    String serverTime = "";
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        viewStudentLiveClassesFrag.findViewById(R.id.cv_no_live_classes).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
//                launchZoomUrl("85780851946");
            }
        });
...
//                getRecordedVideos();
    ...
    void getLiveClasses() {
        ...
        String month = ((StudentLiveClassesTabs)mActivity).currentMonth;
        String year = ((StudentLiveClassesTabs)mActivity).currentYear;
        String fiterdate = ((StudentLiveClassesTabs)mActivity).filterDate;

        Request get = null;

        if (((StudentLiveClassesTabs)mActivity).type == 1) {
        ...
        else  if (((StudentLiveClassesTabs)mActivity).type == 0){
            get = new Request.Builder()
                    .url(AppUrls.GetStudentLiveVideos + "schemaName=" + sh_Pref.getString("schema", "") + "&studentId=" + sObj.getStudentId()+"&sectionId="+sObj.getClassCourseSectionId()+"&filterDate="+fiterdate)
                    .build();

            utils.showLog(TAG, "url - " + AppUrls.GetStudentLiveVideos + "schemaName=" + sh_Pref.getString("schema", "") + "&studentId=" + sObj.getStudentId()+"&sectionId="+sObj.getClassCourseSectionId()+"&filterDate="+fiterdate);
        }
        else {
            get = new Request.Builder()
                    .url(AppUrls.GetStudentLiveVideos + "schemaName=" + sh_Pref.getString("schema", "") + "&studentId=" + sObj.getStudentId()+"&sectionId="+sObj.getClassCourseSectionId()+"&status=active")
                    .build();
            utils.showLog(TAG, "url - " + AppUrls.GetStudentLiveVideos + "schemaName=" + sh_Pref.getString("schema", "") + "&studentId=" + sObj.getStudentId()+"&sectionId="+sObj.getClassCourseSectionId()+"&status=active");
        }
...
//                getRecordedVideos();
    ...
    class LiveAdapter extends RecyclerView.Adapter<LiveAdapter.ViewHolder> {
        ...
        public LiveAdapter.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
            return new LiveAdapter.ViewHolder(LayoutInflater.from(mActivity).inflate(R.layout.item_home_live_class, parent, false));
        }
                ...
                public void onClick(View view) {
                    if (holder.ivLive.getVisibility() == View.VISIBLE) {
                       ...
                       /* Intent zoomActivity = new Intent(getActivity(), ZoomActivity.class);
                        zoomActivity.putExtra(AppConst.MEETING_ID, listUpcoming.get(position).getMeetingId());
                        ...
                        if(listUpcoming.get(position).getMeetingPwd() != null)
                        zoomActivity.putExtra(AppConst.MEETING_PASSWORD, listUpcoming.get(position).getMeetingPwd());
...
//        Intent intent = new Intent(Intent.ACTION_VIEW, Uri.parse("zoomus://narayanagroup.zoom.us/j/6583554590?pwd="));
        ...
        if (isAppInstalled) {
//            Intent intent = new Intent(Intent.ACTION_VIEW, Uri.parse("zoomus://zoom.us/join?confno=85780851946" + "&pwd=UU9IS3A1TXdKbUZ1ekFvbVVZeGFvdz09&uname=sriram"));
