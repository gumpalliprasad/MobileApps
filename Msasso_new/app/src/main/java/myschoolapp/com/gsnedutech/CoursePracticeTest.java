...
import myschoolapp.com.gsnedutech.Models.PracQuestionObjNew;
...
public class CoursePracticeTest extends AppCompatActivity implements View.OnClickListener, NetworkConnectivity.ConnectivityReceiverListener {

    private static final String TAG = "SriRam -" + CoursePracticeTest.class.getName();
...
//    LinearLayout llParagraph;
    ...
    ArrayList<ArrayList<PracQuestionObjNew>> arrayOfArrays = new ArrayList<>();
...
//          timerValue.setText("" + mins + ":"+ String.format("%02d", secs));
    ...
    public void onNetworkConnectionChanged(boolean isConnected) {
        if (!isConnected) {
            ...
            utils.alertDialog(1, CoursePracticeTest.this, getString(R.string.error_connect), getString(R.string.error_internet),
    ...
    void init(){
        ...
        que_NO = getIntent().getIntExtra("que_No", 0);
    ...
    void getQuestionCount(){
        ...
        utils.showLog(TAG, "URL - " + new AppUrls().GetPracticeTestquestionCount +"schemaName=" +sh_Pref.getString("schema","")+ "&contentType=" + getIntent().getStringExtra("contentType") + "&topicId=" + topicId + "&studentId=" + sObj.getStudentId());
        Request request = apiClient.getRequest(new AppUrls().GetPracticeTestquestionCount +"schemaName=" +sh_Pref.getString("schema","")
                + "&contentType=" + getIntent().getStringExtra("contentType")
                + "&topicId=" + topicId +
                "&topicCCMapId=" + topicCCMapId
                + "&studentId=" + sObj.getStudentId(), sh_Pref);
        apiClient.getClient().newCall(request).enqueue(new Callback() {
            ...
            public void onResponse(@NotNull Call call, @NotNull Response response) throws IOException {
                if (response.body()!=null){
                    ...
                    utils.showLog(TAG, "Count - " + jsonResp);

                    try {
                        JSONObject parentjObject = new JSONObject(jsonResp);
                        if (parentjObject.getString("StatusCode").equalsIgnoreCase("200")) {
                            if (parentjObject.has("questionsAnswered")){
                                que_NO = parentjObject.getInt("questionsAnswered");
                                totalQuestions = parentjObject.getInt("totalQuestions");
                            }
                            getPracticeQuestions();
                            ...
                            runOnUiThread(() -> {
                                utils.dismissDialog();
                                MyUtils.forceLogoutUser(toEdit, CoursePracticeTest.this, message, sh_Pref);
                            });
                        }
    ...
    void getPracticeQuestions(){
        ApiClient apiClient = new ApiClient();
        utils.showLog(TAG, "QuesList URL - " + new AppUrls().GetCoursePracticeQueestions + "schemaName=" + getSharedPreferences(AppUrls.shCredentials, MODE_PRIVATE).getString("schema", "") + "&contentType=" + getIntent().getStringExtra("contentType") + "&topicId=" + topicId + "&studentId=" + sObj.getStudentId());
        Request request = apiClient.getRequest(new AppUrls().GetCoursePracticeQueestions + "schemaName=" + getSharedPreferences(AppUrls.shCredentials, MODE_PRIVATE).getString("schema", "")
                + "&contentType=" + getIntent().getStringExtra("contentType")
                + "&topicId=" + topicId
                + "&studentId=" + sObj.getStudentId()+
                "&topicCCMapId=" + topicCCMapId, sh_Pref);
            ...
            public void onResponse(@NotNull Call call, @NotNull Response response) throws IOException {
                if (response.body()!=null){
                    try {
                        ...
                        utils.showLog(TAG, "QuesList response - " + responce);
                        JSONObject parentjObject = new JSONObject(responce);
                        if (parentjObject.getString("StatusCode").equalsIgnoreCase("200")) {
                            JSONArray jsonArr = parentjObject.getJSONArray("studentPracticeTestQuestions");
                            ...
                            utils.showLog(TAG, "MFQFiltered - " + MFQFiltered.length());
                            utils.showLog(TAG, "MFQJSONArray - " + MFQJSONArray.length());

                            // Adding all the Questions other than MFQ's in the Obj Array List
                            List<PracQuestionObjNew> testQueListfromServer = new ArrayList<>();
                            testQueListfromServer.addAll(new Gson().fromJson(MFQFiltered.toString(), new TypeToken<List<PracQuestionObjNew>>() {
                            }.getType()));

//                         Adding all the Questions MFQ's in the Obj Array List
                            List<PracMFQObj> testMFQQueList = new ArrayList<>();
                            ...
                            testQueList.clear();
                            testQueList.addAll(testQueListfromServer);
                            //testQueList.addAll(testMFQQueList);
                            utils.showLog(TAG, "MFQFiltered - testQueList - " + testQueList.size());


                            for (int i = 0; i < testMFQQueList.size(); i++) {
                                PracQuestionObjNew mqo = new PracQuestionObjNew();
                                mqo.setQuestionId(Long.parseLong(testMFQQueList.get(i).getQuestionId()));
                                ...
                                mqo.setTimetaken(testMFQQueList.get(i).getTimetaken());
                                mqo.setQuestionCCMapId(testMFQQueList.get(i).getQuestionCCMapId());
                                mqo.setOption3("NA");
                                mqo.setOption4("NA");
                                mqo.setOption1("NA");
                                mqo.setOption2("NA");
                            ...
                            for (int i = 0; i < testQueList.size(); i++) {
                                if (testQueList.get(i).getQuestType().equalsIgnoreCase("MFQ")) {
                                    testQueList.get(i).setSelectedAnswer("");
                                }
                            }
                            ...
                            runOnUiThread(new Runnable() {
                                ...
                                public void run() {
                                    if (testQueList.size() > 0) {
                                        ...
                                        new AlertDialog.Builder(CoursePracticeTest.this)
                                                .setTitle(getResources().getString(R.string.app_name))
                                                .setMessage("No Data To Load")
    ...
    void postQuestion(String questionId, String timespent, String topicId, String answerType, String value, String questionCCMapId){
        ApiClient apiClient = new ApiClient();
        ...
        try {
            ...
            jsonObject.put("lastQuestionId", questionId);
            jsonObject.put("totalTimeSpent", timespent);
            jsonObject.put("studentId", sObj.getStudentId());
            jsonObject.put("topicId", topicId);
            jsonObject.put("topicCCMapId", topicCCMapId);
            jsonObject.put("updatedBy", sObj.getStudentId());
            jsonObject.put("questionCCMapId", questionCCMapId);
            jsonObject.put("subjectId", subId);
            jsonObject.put("classId", sObj.getClassId());
            jsonObject.put("courseId", sObj.getCourseId());

            jsonObject.put(answerType, value);
        ...
        utils.showLog(TAG, "" + new AppUrls().PostCoursePracticeQueestion);
        utils.showLog(TAG, "" + jsonObject);

        Request request = apiClient.postRequest(new AppUrls().PostCoursePracticeQueestion, body, sh_Pref);
        ...
        apiClient.getClient().newCall(request).enqueue(new Callback() {
            ...
            public void onResponse(@NotNull Call call, @NotNull Response response) throws IOException {
                if (response.body()!=null){
                    ...
                    utils.showLog(TAG, "Post PracQuestionObj Status responce - " + jsonResp);

                    try {
                        JSONObject parentjObject = new JSONObject(jsonResp);
                        if (parentjObject.getString("StatusCode").equalsIgnoreCase("200")) {
                            utils.showLog(TAG, "PracQuestionObj Updated Sucessfully");
                        } else if (parentjObject.has(AppConst.STATUS_CODE) && MyUtils.showLogOutAlert(parentjObject)){ //TODO New Changes
                            String message = parentjObject.getString(AppConst.MESSAGE);
                            runOnUiThread(() -> {
                                utils.dismissDialog();
...
//        Request request = apiClient.postRequest(new AppUrls().GetMockTestQueestions, body);
//        apiClient.getClient().newCall(request).enqueue(new Callback() {
//            @Override
//            public void onFailure(@NotNull Call call, @NotNull IOException e) {
//
...
//                btn[i].setTag("Unselected");
            ...
            btn[i].setOnClickListener(CoursePracticeTest.this);
        }
    ...
    public void loadQuestion(int que_index) {
        ...
        if(que_NO >= totalQuestions){
            que_NO = 1;
        }else
        que_NO = que_NO + 1;
        tvQueNO.setText("Q no: " + que_NO);
...
//        startCountUpTimer();
        ...
        Log.v(TAG, "QueType - " + testQueList.get(que_index).getQuestType());

        if (testQueList.get(que_index).getQuestType().equalsIgnoreCase("MCQ")) {
            ...
            String mainString = "<!DOCTYPE html> <html>" + testQueList.get(que_index).getQuestion()  + testQueList.get(que_index).getOption1() + "</fieldset>"
        ...
        else if (testQueList.get(que_index).getQuestType().equalsIgnoreCase("ITQ")) {
            wvQuestion.setVisibility(View.VISIBLE);
...
//            }
        ...
        else if (testQueList.get(que_index).getQuestType().equalsIgnoreCase("TOF")) {
            llMcq.setVisibility(View.GONE);
        ...
        else if (testQueList.get(que_index).getQuestType().equalsIgnoreCase("FIB")) {
            llITQ.setVisibility(View.GONE);
            wvQuestion.setVisibility(View.VISIBLE);
        ...
        else if (testQueList.get(que_index).getQuestType().equalsIgnoreCase("MFQ")){
            postQuestion("" + testQueList.get(que_index).getQuestionId(), "" + ((SystemClock.elapsedRealtime() - chronometer.getBase()) / 1000),
                    topicId, "correctAnswer", "1", "" + testQueList.get(que_index).getQuestionCCMapId());
    ...
    private void setUnFocusAll() {
        for (Button aBtn : btn) {
            ...
            if (testQueList.get(que_index).getQuestType().equalsIgnoreCase("MAQ"))
...
//                    }
            ...
            case R.id.tv_next:
                chronometer.stop();
                if (testQueList.get(que_index).getQuestType().equalsIgnoreCase("FIB")) {
                    ...
                    if (notans) {
                        Toast.makeText(CoursePracticeTest.this, "Please Answer the Question", Toast.LENGTH_SHORT).show();
                ...
                else if (testQueList.get(que_index).getQuestType().equalsIgnoreCase("MAQ")){
                    showAnswerDialog();
                }
                else {
    ...
    private void showExitDialog() {
        ...
        dialog = new Dialog(CoursePracticeTest.this, android.R.style.Theme_Light_NoTitleBar_Fullscreen);
...
//            tvCans.setText("CorrectAnswer : " + testQueList.get(que_index).getCorrectAnswer());
            ...
            if (ans) {
                ...
                wvCorrect.setText(testQueList.get(que_index).getCorrectAnswer());
                postQuestion("" + testQueList.get(que_index).getQuestionId(), "" + ((SystemClock.elapsedRealtime() - chronometer.getBase()) / 1000),
                        topicId, "wrongAnswer", "1", "" + testQueList.get(que_index).getQuestionCCMapId());
            }
...
//            wvExplanation.setText(selectedOption.getExplanation());
...
//        if the Selected Answer is not Blank
    ...
    public void onBackPressed() {
        setResult(4321,new Intent(this, CourseTopicView.class));
        super.onBackPressed();
